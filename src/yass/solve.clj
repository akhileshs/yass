...
(defn constraints
  ...
  [s i]
  (let [gridList (partition 9 s),       ;; create a list of lists
        row (/ i 9),                    ;; row no
        column (mod i 9),               ;; column no
        columnGroup (/ column 3),            ;; column group no
        rowGroup (/ row 3),               ;; row group no
        every-nth-item (fn [gridList i] (map #(nth % i) gridList))                ;; return every nth item in the list of lists.
        group-column (every-nth-item (map #(partition 3 %) gridList) columnGroup) ;; returns all nos. in the column group
        group (take 3 (drop (* 3 (int rowGroup)) group-column))]                  ;; returns all nos. in the given number's group.
